...
public class Main {
    ...
    // что мои задачи перестали быть тасками, так как были новые методы и логика в них. Я придумала как мне показалось
    // очень интересную штуку, если я не могу трансформировать вои таски, я переделаю их в то, что смогу трансформировать
    ...
    // все шло отлично GET запросы на ура пошли, DELETE тоже, однако когда нужн было из этого формата вернуть данные в тааски
    // то тут начались действительно проблемы, приходилось много раз все переписывать, в общем идея может и не самая лучша
